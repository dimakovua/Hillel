...
GameField::GameField()
{
    ...
    setFocusPolicy(Qt::StrongFocus); // активным будет виджет - отлавливает нажатия клавиш
    m_snakeItemSize = 10;
...
void GameField::paintEvent(QPaintEvent *e) {
    ...
    for (int i = 0; i < m_snake -> m_snakeBody.size(); i++)
    {
        painter.drawEllipse(m_snake->m_snakeBody[i] ->m_x * m_snakeItemSize,
                            m_snake->m_snakeBody[i] ->m_y * m_snakeItemSize, m_snakeItemSize, m_snakeItemSize);
    }
...
void GameField::MoveSnakeSlot()
{
    ...
    {
        newSnakeItem = new SnakeItem (m_snake->m_snakeBody[0] -> m_x +1,
                                     m_snake->m_snakeBody[0] ->m_y);
    }
    ...
    // проигрыш
    // проигрыш
    for (int i = 1; i < m_snake->m_snakeBody.size(); i++) // начинаем с 1, а не 0
    {
        if (newSnakeItem->m_x == m_snake->m_snakeBody[i]->m_x &&
            newSnakeItem->m_y == m_snake->m_snakeBody[i]->m_y)
        {
            GameOver();
            return; // завершить обработку, чтобы не продолжать движение змеи
        }
    }
